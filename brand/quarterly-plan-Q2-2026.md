# Weekly Content Plan: March – May 2026

> Capable Wealth — Precision planning for life beyond the OR.

This plan drills the editorial plan's March–May 2026 slate into 13 weekly content assignments across all channels. Each week is organized around one anchor piece that sets the theme; LinkedIn, YouTube, and Facebook content amplifies and extends it. Content flows week to week — not random, but sequential and cumulative.

**Source documents:**
- [editorial-plan-2026.md](editorial-plan-2026.md) — The 12-month backbone
- [content-recipe.md](content-recipe.md) — Production workflow, building blocks, templates
- [content-calendar.md](content-calendar.md) — Static cycles, research checkpoints, signal triggers
- [voice-profile.md](voice-profile.md) — Jared's voice, philosophy, structural DNA

---

## Quarter at a Glance

| Month | Theme | Weeks | Key Deadlines |
|-------|-------|-------|---------------|
| March | Q1 Financial Health Check — Are You Paying Yourself the Right Amount? | 1–4 | April 15 approaching; CMS 2.5% cut active since Jan 1 |
| April | Post-Filing Insights — What Your Return Just Told You | 5–8 | April 15 filing/payment/IRA deadline; Q1 estimated tax |
| May | The Exit Conversation — Practice Transition Starts Earlier Than You Think | 9–13 | Conference season (AAOS, regionals); PE deal activity high |

### Content Volume Summary

| Channel | Pieces per Week | 13-Week Total |
|---------|----------------|---------------|
| Anchor (blog/article) | 1 | 13 |
| Video podcast | 1 | 13 |
| Short-form clips (from podcast) | 2-5 (varies) | ~26-65 |
| LinkedIn posts | 3 | 39 |
| Facebook posts | 3 | 39 |
| **Core total** | **8** | **~104** |
| **Including clips** | **10-13** | **~130-169** |

---

## MARCH 2026: Q1 Financial Health Check

> **Surgeon context:** Full surgical schedules — March is one of the busiest OR months. Content must be concise and high-impact. Surgeons are heads-down on patient care, but Q1 revenue data is emerging and the April 15 deadline looms. CMS's 2.5% "efficiency adjustment" is now active and surgeons are feeling it in their reimbursements.

---

### Week 1: February 23 – March 1

#### Theme: The structure problem, not the tax problem

---

**ANCHOR PIECE**

- **Working title:** "You're Not Overtaxed. You're Understructured."
- **Target angle & key argument:** Most orthopedic surgeons fixate on their tax bill and assume they need a "better accountant." The real driver of how much they keep comes down to their entity structure. Walk through the three structural gaps that cost the most: (1) a misaligned S-Corp salary, (2) an underfunded or nonexistent cash balance plan, and (3) a missing cost segregation study on owned practice or investment real estate. The argument: fixing structure is worth more than chasing deductions.
- **Math example:** An $800K-earning orthopedic surgeon with an unoptimized S-Corp salary of $400K, no cash balance plan, and no cost segregation study. Show the annual leakage: ~$25K from excess self-employment tax on the mis-set salary, ~$45K–$80K in lost tax-deferred compounding from the missing cash balance plan, and ~$15K–$30K in missed depreciation from the absent cost seg study. Total: $85K–$130K left on the table annually through structural neglect.
- **Structural approach:** Contrarian Reframe Opening — challenge the "I'm overtaxed" complaint, then pivot to the Diagnostic Framework mirroring a clinical assessment: symptoms (high tax bill), diagnosis (structural misalignment), treatment plan (three specific interventions). Close with the philosophical tie-back: structure is control, and control is what separates wealth from high income.

---

**LINKEDIN POST 1**

- **Hook line:** "Every week, I sit down with a surgeon who's frustrated about taxes. And every week, the conversation ends in the same place: their entity structure."
- **Core insight:** The difference between an $800K earner who keeps $480K and one who keeps $560K comes down to three structural decisions within their entity, not a better CPA.
- **Relationship to anchor:** Distillation of the anchor's central argument into a single LinkedIn-length insight. Drives traffic to the full piece.

**LINKEDIN POST 2**

- **Hook line:** "I reviewed a surgeon's financials last month. He was leaving $130K on the table. Every year. For six years."
- **Core insight:** Three structural gaps — S-Corp salary calibration, cash balance plan absence, and missing cost segregation — account for most of the wealth leakage in surgeon practices.
- **Relationship to anchor:** Tells a specific client-story version of the anchor's framework. Hooks through narrative rather than argument.

**LINKEDIN POST 3**

- **Hook line:** "A cash balance plan can shelter $150K–$350K per year in tax-deferred contributions. Most surgeons over 50 don't have one."
- **Core insight:** The most powerful wealth-building tool for high-income practice owners is the one their CPA probably hasn't mentioned. The cash balance plan is the single highest-leverage structural addition for surgeons earning $700K+.
- **Relationship to anchor:** Zooms in on one of the three structural gaps from the anchor — the cash balance plan — and makes it the standalone hero.

---

**VIDEO PODCAST CONCEPT**

- **Topic and angle:** "The Three Leaks Draining Your Surgeon Paycheck" — a visual walkthrough of the three structural gaps, using a "bucket with holes" metaphor.
- **Contrarian hook (first 15 seconds):** "If you make $800K a year and feel like you should have more to show for it... you're right. The answer comes down to three holes in your financial structure that nobody has told you about."
- **Key talking points:**
  1. The S-Corp salary trap — how the "reasonable compensation" your CPA set five years ago is probably wrong in both directions
  2. The cash balance plan gap — the $150K–$350K shelter most surgeons miss
  3. The cost segregation blind spot — accelerated depreciation on real estate you already own
  4. Why CPAs aren't the villain — their job is compliance, not optimization
  5. The cumulative cost: show the 10-year impact of these three gaps at surgeon income levels
- **Philosophical close:** "You didn't spend a decade in training to be efficient in the OR and careless with everything that comes after. The same precision you bring to a procedure — that's what your financial structure deserves."

**CLIP CANDIDATES**

1. **The cash balance plan reveal** (talking point 2): The $150K-$350K shelter stat delivered cold. Hook: "In 2026, the most you can put into a 401(k) is $32,500. But there's another vehicle..." ~50s `[BOTH]`
2. **The cumulative cost** (talking point 5): Add up all three leaks to $85K-$130K/year, then show the 10-year number. Hook: "Let me add up what these three structural gaps are actually costing..." ~55s `[BOTH]`
3. **CPAs aren't the villain** (talking point 4): The compliance vs. optimization distinction. Hook: "This is not your CPA's fault..." ~45s `[YT-SHORT]`

---

**FACEBOOK POST 1**

- **Core idea:** Quick, conversational version of the anchor's thesis — your tax frustration is really a symptom of a structure that hasn't been updated since your CPA set it up. One question to ask your advisor this week: "When was the last time we stress-tested my entity structure?"
- **Tone adjustment notes:** Warmer and more personal than LinkedIn. Open with something like "I had a conversation this week that I keep coming back to..." Avoid listing all three gaps — just tease the idea and invite curiosity.

**FACEBOOK POST 2**

- **Core idea:** Lighter retelling of the LinkedIn 2 client story — a surgeon leaving $130K/year on the table for six years. Focus on the human moment of realization, not the full diagnostic. The emotional center: "The hard part? Nobody had ever looked."
- **Tone adjustment notes:** Story-driven and personal. Keep the three gaps implied rather than listed. Let the narrative carry the message.

**FACEBOOK POST 3**

- **Core idea:** Conversational nudge about the cash balance plan — most surgeons are contributing $32,500/year when they could be contributing $150K–$350K. Frame as a "did you know" moment, not a lesson. Close with the time-sensitive setup window for 2026 contributions.
- **Tone adjustment notes:** Curious, warm, invitational. Don't over-explain the mechanics. Create awareness and let the numbers speak.

---

**Timing rationale:** Publish the anchor piece in the last week of February to set the Q1 health check theme early. Tax-related content must be out by mid-March to give surgeons time to act before April 15. Starting with the structural message before March even begins gives the rest of the quarter's content a foundation to build on.

---

### Week 2: March 2–8

#### Theme: The S-Corp salary question

---

**ANCHOR PIECE**

- **Working title:** "The S-Corp Salary Trap: Why Your CPA's 'Reasonable Compensation' Might Be Costing You"
- **Target angle & key argument:** S-Corp reasonable compensation review window opens in Q1, and most surgeon-owners are running on a number their CPA set years ago. Challenge the assumption that "reasonable" means "optimal." The S-Corp salary is a balancing act: set it too high and you overpay self-employment tax; set it too low and you limit retirement plan contributions (which are based on W-2 compensation). At $745K+ in total income, getting this wrong in either direction costs real money — and most surgeons have never been shown the math on the trade-off.
- **Math example:** A surgeon with $745K in practice income. Scenario A: S-Corp salary set at $300K (the "low" default). Self-employment tax savings are maximized, but the 401(k) employer match and cash balance plan contributions are capped by the lower W-2. Scenario B: Salary set at $360K (the IRS compensation cap). Higher self-employment tax, but unlocks an additional $35K–$60K in retirement plan contributions depending on age and cash balance plan design. Net benefit of Scenario B: ~$15K–$25K per year in additional tax-deferred compounding, even after the higher payroll tax cost. The "lower salary saves money" assumption is wrong once retirement plan math enters the picture.
- **Structural approach:** Conversation Hook — open with a conversation with a surgeon client who says "my CPA told me to keep my salary as low as possible." Then Reframe: "Your CPA is optimizing for one variable. There are three." Core Teaching via a decision framework: the three variables (payroll tax, retirement contributions, QBI deduction) and how they interact. Math example as the persuasion anchor. Close with: the right salary is the one that puts the most money to work.

---

**LINKEDIN POST 1**

- **Hook line:** "Your CPA told you to keep your S-Corp salary low. That advice might be costing you $25K a year."
- **Core insight:** The "low salary" strategy optimizes for payroll tax savings but sacrifices retirement plan contribution capacity — which is worth more at surgeon income levels.
- **Relationship to anchor:** Direct distillation of the anchor's core argument. The numbers shock; the full piece explains.

**LINKEDIN POST 2**

- **Hook line:** "There are three numbers fighting for control of your S-Corp salary. Most surgeons only know about one of them."
- **Core insight:** The S-Corp salary decision involves a three-way trade-off: payroll taxes, retirement plan contributions, and the QBI deduction. Optimizing for just one — which is what most CPAs do — leaves money on the table.
- **Relationship to anchor:** Teases the decision framework from the anchor without giving away the full resolution. Curiosity driver.

**LINKEDIN POST 3**

- **Hook line:** "There's a deduction worth up to $29,800 per year for a surgeon earning $745K. Most practice owners either don't have it or have a smaller version of it than they should."
- **Core insight:** The QBI deduction (20% of pass-through income) depends directly on the S-Corp salary ratio. Set the salary too high and you shrink pass-through income; too low and you trigger the W-2 wage limitation. There's a calibrated range in the middle where the deduction is maximized, and most surgeons have never modeled it.
- **Relationship to anchor:** Isolates the least understood of the three S-Corp salary variables — the QBI deduction — as a standalone deep-dive. Completes the LinkedIn arc from provocation (Post 1) to framework (Post 2) to hidden variable (Post 3).

---

**VIDEO PODCAST CONCEPT**

- **Topic and angle:** "The Salary You Pay Yourself Could Be Your Most Expensive Financial Decision" — show the whiteboard math on the S-Corp salary trade-off in real time.
- **Contrarian hook (first 15 seconds):** "What if I told you that the salary you're paying yourself — the number you've barely thought about since your CPA set it up — could be the single most impactful number in your entire financial plan? Not your investment returns. Not your tax rate. Your salary."
- **Key talking points:**
  1. Why most CPAs default to "keep it low" — they're optimizing for compliance, not wealth building
  2. The payroll tax math at $300K vs. $360K — the difference is real but smaller than you think
  3. The retirement plan unlock — how a higher salary feeds 401(k) + cash balance contributions
  4. The QBI deduction wrinkle — how the 20% pass-through deduction interacts with salary
  5. The annual review cadence: this isn't a set-it-and-forget-it number
- **Philosophical close:** "Control over your financial life starts with understanding the numbers your own structure is built on. Most surgeons can describe the biomechanics of every implant they use — but they can't explain why their salary is what it is. That's a gap worth closing."

**CLIP CANDIDATES**

1. **The payroll tax math** (talking point 2): $300K vs. $360K comparison delivered as a quick reveal. Hook: "Your CPA told you to keep your S-Corp salary low. Here's what that's actually costing you." ~55s `[BOTH]`
2. **The retirement plan unlock** (talking point 3): How a higher salary feeds $20K-$60K more into retirement. Hook: "A $60K salary adjustment just unlocked $60K in retirement contributions. Same income. Different structure." ~50s `[YT-SHORT]`
3. **The QBI deduction wrinkle** (talking point 4): The hidden third variable most surgeons don't know exists. Hook: "There's a deduction worth up to $29,800 that depends entirely on one number you've never optimized." ~60s `[IG-REEL]`

---

**FACEBOOK POST 1**

- **Core idea:** "One question every surgeon practice owner should ask their CPA this quarter: When was the last time we recalculated my S-Corp salary against my retirement plan capacity?" Frame it as a simple action item — not a deep dive.
- **Tone adjustment notes:** Keep it short and invitational. Don't explain the full S-Corp mechanics. Just create awareness that this is a lever worth pulling.

**FACEBOOK POST 2**

- **Core idea:** Conversational version of the three-variable framework from LinkedIn 2. Your S-Corp salary isn't one decision, it's three: payroll taxes, retirement contributions, and the QBI deduction. Most practices only optimize for one.
- **Tone adjustment notes:** Direct but warm. Use a simple numbered list for the three variables. Keep it to the concept without the full math. Close with "worth a closer look this quarter."

**FACEBOOK POST 3**

- **Core idea:** End-of-week reflective wrap tying Weeks 1 and 2 together. Two weeks of content boiled down to one message: the structure you set up years ago probably isn't the right structure today, and Q1 is the natural time for a coordinated review of entity, salary, retirement plans, and tax strategy.
- **Tone adjustment notes:** Reflective, invitational, forward-looking. Frame Q1 as the natural checkpoint. Close with a soft CTA: one coordinated conversation before April 15.

---

**Timing rationale:** Week 2 builds on Week 1's structural theme by zooming into the single most common structural misalignment — the S-Corp salary. Q1 is the natural review window for reasonable compensation. Publishing this in the first week of March gives surgeons (or their advisors) 6 weeks to recalibrate before April 15.

---

### Week 3: March 9–15

#### Theme: The reimbursement shift and what it means for your wealth

---

**ANCHOR PIECE**

- **Working title:** "The 2.5% Cut Nobody's Talking About — and What It Actually Means for Your Practice"
- **Target angle & key argument:** CMS's 2026 "efficiency adjustment" cuts reimbursement on non-time-based orthopedic procedures by 2.5%. Most surgeons will hear about this from their practice manager, not their financial advisor — and most will respond by trying to increase volume. Challenge that reflex. When revenue per procedure drops, the strategic response isn't more procedures — it's restructuring how practice income flows through to personal wealth. Frame the reimbursement cut as a strategic inflection point: a signal to evaluate practice overhead, optimize entity structure, and ensure every dollar of reduced revenue is being kept as efficiently as possible.
- **Math example:** A surgeon performing 400 procedures/year at an average reimbursement of $3,200. The 2.5% cut reduces per-procedure revenue by $80, totaling $32,000 in annual revenue loss. If the surgeon responds by adding 10 procedures to make up the difference, the marginal cost of those additional procedures (OR time, anesthesia, supplies, staff) consumes ~60% of the revenue — netting only $12,800 recovered. Compare this to a structural optimization: reducing practice overhead by 2% (realistic for most practices) recovers the full $32K with zero additional procedures. The point: working harder is the most expensive way to replace lost revenue.
- **Structural approach:** The Diagnostic Framework — open with the CMS announcement as the "imaging result," then diagnose the real problem (most surgeons will treat the symptom — lost revenue — rather than the underlying condition — structural inefficiency). Core Teaching walks through three response strategies: (1) volume increase (the instinctive but expensive response), (2) overhead optimization (the overlooked middle ground), and (3) income flow restructuring (the highest-leverage move). Tie back to the thesis: reimbursement cuts don't change how much you earn as much as they reveal how much you're wasting.

---

**LINKEDIN POST 1**

- **Hook line:** "CMS cut your reimbursement by 2.5%. Your practice manager told you. Your financial advisor probably didn't."
- **Core insight:** The 2.5% "efficiency adjustment" hitting orthopedic procedures is a financial planning event, not just a practice management headache. The response should come from your financial structure, not your surgical schedule.
- **Relationship to anchor:** Frames the reimbursement cut as a financial planning issue — the same structural lens from Weeks 1 and 2.

**LINKEDIN POST 2**

- **Hook line:** "Adding 10 procedures to offset a $32K revenue loss sounds logical. Here's why it costs you more than it recovers."
- **Core insight:** The marginal cost of additional procedures (OR time, supplies, staff) consumes most of the revenue they generate. Structural optimization recovers lost revenue more efficiently than volume increases.
- **Relationship to anchor:** Presents the math comparison from the anchor as a standalone insight. The numbers do the persuading.

**LINKEDIN POST 3**

- **Hook line:** "OBBBA tied Medicare physician payments to 75% of the MEI in 2026. Here's what that number means in your pocket."
- **Core insight:** The One Big Beautiful Bill Act's Medicare payment formula means reimbursement will continue to lag inflation for the foreseeable future. This isn't a one-year adjustment — it's a structural shift in practice economics that requires a structural response.
- **Relationship to anchor:** Extends the anchor's argument from the immediate 2.5% cut to the longer-term policy trajectory. Creates urgency without fear-mongering.

---

**VIDEO PODCAST CONCEPT**

- **Topic and angle:** "Your Practice Makes Less Per Procedure Than It Did Last Year. Here's What to Do About It." — walk through the reimbursement math and the three response strategies.
- **Contrarian hook (first 15 seconds):** "Most orthopedic surgeons I talk to are planning to respond to the CMS reimbursement cut by doing more surgery. That's like treating a fracture by asking the patient to walk faster. The problem isn't volume — it's structure."
- **Key talking points:**
  1. What the 2.5% efficiency adjustment actually means for non-time-based orthopedic procedures
  2. The math on volume replacement vs. structural optimization
  3. Three overhead line items most practices have never benchmarked (supply costs, staffing ratios, facility utilization)
  4. How the OBBBA payment formula creates a multi-year reimbursement headwind
  5. The strategic question: is your practice structured to build wealth or just to generate revenue?
- **Philosophical close:** "You wouldn't treat every patient with the same implant regardless of the pathology. Don't treat every financial challenge with the same response — more hours, more procedures, more volume. Sometimes the most productive thing you can do is stop and look at the structure."

**CLIP CANDIDATES**

1. **The volume trap math** (talking point 2): 10 extra procedures to recover $32K, but only $12,480 nets. Hook: "Adding 10 procedures to offset a $32K revenue loss sounds logical. Here's why it costs you more than it recovers." ~55s `[BOTH]`
2. **The three overhead benchmarks** (talking point 3): Quick reveal of supply costs, staffing ratios, facility utilization as untouched levers. Hook: "Three line items in your practice financials that nobody has ever benchmarked." ~60s `[IG-REEL]`
3. **Revenue vs. wealth** (talking point 5): Two surgeons, similar revenue, $80K/year gap in take-home. Hook: "Surgeon A collects $1.4 million. Surgeon B collects $1.35 million. Surgeon B takes home $80K more." ~55s `[BOTH]`

---

**FACEBOOK POST**

- **Core idea:** "I keep hearing from surgeons that the CMS reimbursement cuts feel like death by a thousand cuts. They're not wrong. But here's the thing — the response most surgeons default to (do more procedures) is the least efficient way to recover lost revenue. There's a better path. It starts with looking at how your practice income flows, not how much of it there is."
- **Tone adjustment notes:** More empathetic than LinkedIn. Acknowledge the frustration before pivoting to the structural reframe. No math in the Facebook version — just the conceptual shift.

---

**Timing rationale:** Early-to-mid March is the right moment for this piece. The CMS cuts have been active since January 1 — surgeons have now seen 10 weeks of reduced reimbursements and are starting to feel it in their Q1 numbers. Publishing before the conversation goes stale, while the pain is fresh and actionable.

---

### Week 4: March 16–22

#### Theme: The number that matters more than collections

---

**ANCHOR PIECE**

- **Working title:** "Before Q1 Closes: The One Number That Matters More Than Collections"
- **Target angle & key argument:** Surgeons track collections religiously — it's the headline number in every practice meeting. But collections tell you how much the practice earned, not how much the surgeon kept. Introduce the "surgeon effective hourly rate" — a single number that captures what remains after taxes, overhead, debt service, and mandatory savings. If a practice collects $2.4M annually but the surgeon's take-home after everything is $380K, and the surgeon works 2,600 clinical hours, the effective rate is $146/hour. Compare that to what the surgeon assumes they're earning. The reframe: you don't have a revenue problem — you have a keep rate problem. Position this as the framework to apply as soon as Q1 data is available at month-end.
- **Math example:** A $2.4M-collecting practice with 55% overhead ($1.32M), leaving $1.08M for the surgeon. After federal + state taxes (~40% effective on $1.08M = $432K), mandatory retirement contributions ($72K), and debt service ($96K/year on practice loans), the surgeon keeps ~$480K. At 2,600 clinical hours/year, the effective hourly rate is $185/hour. Now model the same surgeon with structural improvements from Weeks 1–3: overhead reduced to 52% ($1.25M), S-Corp salary optimized, cash balance plan added. After-tax keep rate improves to ~$560K, effective hourly rate rises to $215/hour. The $30/hour difference — over 2,600 hours — is $78,000 annually.
- **Structural approach:** Math-First Approach — open with the collections number everyone celebrates, then immediately introduce the effective hourly rate calculation that deflates it. The contrast creates the hook. Core Teaching walks through the five components that determine the effective rate (gross collections, overhead, taxes, mandatory savings, debt service) and which ones the surgeon can actually influence. Close with the Q1 checkpoint: when Q1 closes at the end of March, calculate your effective hourly rate and compare it to last year. If it's lower, the CMS cuts and structural gaps from the previous three weeks are the likely culprits. Give surgeons the framework now so they're ready to run the numbers as soon as Q1 data is available.

---

**LINKEDIN POST 1**

- **Hook line:** "Your practice collected $2.4M last year. You kept $480K. Your effective hourly rate? $185."
- **Core insight:** Collections measure the practice's health. The surgeon's effective hourly rate measures the surgeon's wealth trajectory. They're not the same number — and most surgeons have never calculated the second one.
- **Relationship to anchor:** Presents the anchor's key calculation as a provocation. The gap between perceived earnings and effective rate is the hook.

**LINKEDIN POST 2**

- **Hook line:** "Q1 closes in two weeks. Before you celebrate (or panic) over the top-line number, ask this: did your keep rate go up or down?"
- **Core insight:** Revenue growth without structural improvement just means the practice is generating more income for everyone except the surgeon. The keep rate — what percentage of collections the surgeon actually retains — is the Q1 vital sign that matters.
- **Relationship to anchor:** Positions the keep rate as the Q1 health check metric, tying the month's theme into a single actionable diagnostic.

---

**VIDEO PODCAST CONCEPT**

- **Topic and angle:** "Before Q1 Closes: The One Number That Matters More Than Collections" — the editorial plan's designated YouTube piece for March, adapted as a visual walkthrough of the effective hourly rate calculation.
- **Contrarian hook (first 15 seconds):** "Every surgeon I know can tell me their practice's gross collections within $50K. Almost none of them can tell me their effective hourly rate — the actual dollar amount they earn for each hour spent in clinical work, after taxes, overhead, debt, and savings. That gap between what you think you make and what you actually keep is where wealth disappears."
- **Key talking points:**
  1. Why collections is a vanity metric for the surgeon (it measures practice health, not surgeon wealth)
  2. The five components of the effective hourly rate: gross revenue, overhead, taxes, retirement, debt service
  3. How to calculate yours in 15 minutes as soon as Q1 data is available
  4. What the number should be vs. what it usually is at the $800K compensation level
  5. The three levers that move the rate: overhead (Week 3), structure (Week 1), and time allocation
- **Philosophical close:** "You measure outcomes in the OR because you know that what gets measured gets improved. The same principle applies to your finances. If you've never calculated your effective hourly rate — the real measure of what your time is worth — this is the week to do it. Not because the number will be comfortable, but because it will be true."

**CLIP CANDIDATES**

1. **The effective hourly rate reveal** (talking point 1-2): Collections of $2.4M, but effective rate is $185/hour. Hook: "Your practice collected $2.4M last year. You kept $480K. Your effective hourly rate? $185." ~55s `[BOTH]`
2. **The $30/hour structural gap** (talking point 5): Same surgeon, structural improvements move rate from $185 to $215/hour, worth $78K/year. Hook: "A $30/hour difference doesn't sound like much. Over 2,600 clinical hours, it's $78,000." ~50s `[YT-SHORT]`
3. **The 15-minute calculation** (talking point 3): Walk through how to calculate the number in 15 minutes with Q1 data. Hook: "Here's how to calculate your real hourly rate in 15 minutes." ~75s `[IG-REEL]`

---

**FACEBOOK POST 1**

- **Core idea:** "I've been working with surgeons long enough to know that most of them can quote their collections number but have never calculated what they actually keep per hour. This week, one surgeon ran the math and the look on his face told me everything. The number was $146/hour — not the $350+ he'd assumed. That gap is a diagnosis, and like any good diagnosis, it points directly to the treatment plan."
- **Tone adjustment notes:** Story-driven, personal. Make it about the human reaction to the number, not the math itself. Warm, not clinical.

**FACEBOOK POST 2**

- **Core idea:** End-of-month wrap: "March is almost over. If you take one thing from the last four weeks, let it be this: the amount your practice earns and the amount you keep are two very different numbers. Understanding the gap between them is where real financial planning begins."
- **Tone adjustment notes:** Reflective, brief. Closing out the March theme with a single takeaway.

---

**Timing rationale:** Mid-March — Q1 is approaching its close but hasn't ended yet. Publishing the effective hourly rate framework now gives surgeons the tool before the data arrives: when Q1 closes on March 31, they're primed to run the calculation immediately. This positions them to act rather than react. Sets up April's post-filing theme: if March was about diagnosing the structural gaps, April is about what the tax return reveals.

---

## APRIL 2026: Post-Filing Insights

> **Surgeon context:** Many surgeons filed extensions. Those who filed are getting returns back. April 15 brings the Q1 estimated tax payment and the prior-year IRA contribution deadline. Content should normalize extensions and reframe tax filing as a diagnostic tool, not a chore. All April 15 action content must publish in the first two weeks to give surgeons time to act.

---

### Week 5: March 23–29

#### Theme: Your tax return is a diagnostic report

---

**ANCHOR PIECE**

- **Working title:** "Your Tax Return Is a Diagnostic Report. Here's How to Read It."
- **Target angle & key argument:** Surgeons understand diagnostic imaging — they don't just look at the image, they interpret what it means for treatment. Apply the same framework to a tax return. Walk through 3–4 specific line items on a surgeon's return that reveal structural problems: the effective tax rate line (is it higher than it should be at your income level?), the QBI deduction (present or absent — and why), the retirement contribution total vs. what was available (how much capacity went unused?), and the state tax burden (is your state costing you more than you think?). The return is a compliance document for your CPA. For you, it should be a strategic planning tool.
- **Math example:** A surgeon's return showing $820K in AGI, $298K in total tax (36.3% effective rate), a QBI deduction of $0 (because their S-Corp salary was too high relative to pass-through income), retirement contributions of $31,000 (the 401(k) max but no cash balance plan), and $52K in state income tax. Diagnosis: the effective rate suggests structural optimization could reduce it to 31–33% (saving $25K–$40K). The missing QBI deduction points to an S-Corp salary calibration issue. The retirement contribution gap — $31K contributed vs. $200K+ available with a cash balance plan — represents the single largest missed opportunity at $169K+ in lost tax-deferred compounding.
- **Structural approach:** The Diagnostic Framework — explicitly mirror how a surgeon reads an MRI. "Just as you don't look at an image and see 'a knee,' you read specific findings that tell a story. Your tax return works the same way." Core Teaching uses four "findings" (effective rate, QBI deduction, retirement contributions, state burden) as diagnostic markers, each with a "normal range," a "concerning finding," and a "treatment recommendation." This mirrors clinical thinking and makes the content feel natural to the surgeon audience.

---

**LINKEDIN POST 1**

- **Hook line:** "Your tax return is a 30-page diagnostic report. Your CPA read it for compliance. Nobody read it for strategy."
- **Core insight:** Four line items on your return — effective rate, QBI deduction, retirement contributions, and state tax — tell you whether your financial structure is working or leaking.
- **Relationship to anchor:** Direct compression of the anchor's framework into a single insight. Teases the "four findings" without explaining them.

**LINKEDIN POST 2**

- **Hook line:** "A surgeon's return showed $31,000 in retirement contributions. The available capacity was $203,000. That $172,000 gap compounds — every single year."
- **Core insight:** The retirement contribution line on your tax return is the clearest indicator of whether you're using your full tax-sheltered capacity. Most surgeons use less than 20% of what's available to them.
- **Relationship to anchor:** Isolates the most visceral of the four diagnostic markers — the retirement gap — and lets the number speak.

**LINKEDIN POST 3**

- **Hook line:** "The extension isn't procrastination. It's a strategy — if you use it right."
- **Core insight:** More than half of high-income taxpayers file extensions. The 6-month window from April 15 to October 15 is the most valuable planning period of the year — but only if you treat it as optimization time, not avoidance. Specific moves available during the extension window: S-Corp salary adjustment, retirement plan establishment, entity restructuring analysis.
- **Relationship to anchor:** Extends the anchor's "return as diagnostic" message to those who haven't filed yet — the extension window IS the treatment planning period.

---

**VIDEO PODCAST CONCEPT**

- **Topic and angle:** "How to Read Your Tax Return Like a Surgeon Reads an MRI" — visual walkthrough of a (redacted, hypothetical) surgeon tax return highlighting the four diagnostic markers.
- **Contrarian hook (first 15 seconds):** "You spent 10+ years learning to read diagnostic images. You can look at an MRI and see what everyone else misses. But I'm willing to bet you've never looked at your own tax return with that same diagnostic eye. You hand it to your CPA. They file it. You move on. That 30-page document is trying to tell you something — and nobody's reading it."
- **Key talking points:**
  1. The effective tax rate line — what "normal" looks like at $800K and what a high reading means
  2. The QBI deduction — why it's there (or not) and what that reveals about S-Corp calibration
  3. The retirement contribution total — the gap between what you contributed and what was available
  4. The state tax burden — when your state becomes a wealth drag worth addressing
  5. The "treatment plan" — one action to take based on each finding
- **Philosophical close:** "A diagnosis without a treatment plan is just information. The same is true for your tax return. The return tells you what happened. What you do with it determines what happens next."

**CLIP CANDIDATES**

1. **The $172K retirement gap** (talking point 3): $31K contributed vs. $203K available, delivered as a single devastating number. Hook: "A surgeon's return showed $31,000 in retirement contributions. The available capacity was $203,000." ~50s `[BOTH]`
2. **The effective tax rate diagnostic** (talking point 1): What "normal" looks like at $800K and what a high reading signals. Hook: "Your effective tax rate should be between 31% and 33% at $800K. If it's 36%+, your structure is leaking." ~55s `[YT-SHORT]`
3. **The MRI metaphor** (contrarian hook + talking point 5): The full diagnostic parallel, condensed. Hook: "You spent 10 years learning to read diagnostic images. Have you ever read your own tax return with the same eye?" ~65s `[IG-REEL]`

---

**FACEBOOK POST**

- **Core idea:** "Tax season confession: the first time I really looked at a surgeon client's return — not for compliance, but for strategy — I found $172,000 in unused retirement plan capacity. Every year. For years. It wasn't that they were doing anything wrong. It's that nobody had ever read the return the way a surgeon reads an MRI — looking for what's not there as much as what is."
- **Tone adjustment notes:** Story-based, confessional. The "confession" framing makes it personal. Keep the math to one number — the $172K gap. Let the story do the work.

---

**Timing rationale:** Last week of March — three weeks before the April 15 deadline. Surgeons who filed early are receiving returns. Surgeons who haven't must decide by April 15 whether to file or extend. Publishing the diagnostic framework now gives maximum lead time to act on the findings before the deadline.

---

### Week 6: March 30 – April 5

#### Theme: The April 15 trifecta — filing, estimated taxes, and the Roth question

---

**ANCHOR PIECE**

- **Working title:** "The Backdoor Roth Window Closes April 15 — But the Real Question Is Whether It Still Makes Sense for You"
- **Target angle & key argument:** The prior-year IRA contribution deadline creates urgency, but not every surgeon should reflexively fund a backdoor Roth. With OBBBA making the 37% top rate and current tax framework permanent, the traditional calculus on Roth vs. traditional has shifted. If you're paying 37% now and expect to be in a lower bracket in retirement, the Roth conversion means paying taxes at the peak rate and withdrawing at a lower one — mathematically suboptimal. Walk through the decision tree: current marginal rate vs. expected retirement rate, state tax trajectory (are you moving from a high-tax to a no-tax state?), estate planning goals (Roth IRAs have no RMDs and pass tax-free), and the interaction with other strategies (would that $7,500 be better deployed toward a cash balance plan increase or DAF contribution?).
- **Math example:** A 55-year-old surgeon at 37% federal + 5% state = 42% combined marginal rate. Backdoor Roth contribution of $7,500 has already been taxed at 42%. If the surgeon retires at 62 in a no-income-tax state and withdraws at an effective rate of 24%, the Roth saves 0% (the money was taxed going in, not coming out — it's a wash on the $7,500 contribution itself, but the growth is tax-free). Compare: the same $7,500 directed to a cash balance plan increase shelters the full amount at 42%, saving $3,150 immediately. Over 10 years of tax-deferred compounding at 6%, that single redirect grows to $5,640 in tax savings. The answer isn't "always do the Roth" — it's "model both and choose deliberately."
- **Structural approach:** Decision Tree — open with the reflexive "I always do a backdoor Roth" assumption, then challenge it: "OBBBA changed the math. Have you updated your thinking?" Core Teaching presents a visual decision tree with three branches: (1) Roth makes sense (low current rate, high expected retirement rate, estate planning priority), (2) Traditional/cash balance makes sense (high current rate, lower expected retirement rate, tax deferral priority), (3) It depends on state tax trajectory. Each branch includes specific criteria and the math to support it.

---

**LINKEDIN POST 1**

- **Hook line:** "You've been funding a backdoor Roth every year because someone told you to. Here's why the math might have changed."
- **Core insight:** OBBBA made the 37% top rate permanent. If you're paying 37% now and expect to pay less in retirement, the backdoor Roth may be the most expensive way to save $7,500.
- **Relationship to anchor:** Distills the anchor's central challenge — the Roth reflexive habit — into a provocative one-liner.

**LINKEDIN POST 2**

- **Hook line:** "Estimated taxes aren't a quarterly bill. They're a quarterly strategy session."
- **Core insight:** The Q1 estimated tax payment due April 15 is an opportunity to recalibrate — adjusting withholding, timing income, and pacing retirement contributions. Introduce a "quarterly financial vital signs" framework: each payment is a moment to check four numbers (year-to-date income vs. projection, retirement contribution pacing, estimated tax adequacy, and cash reserve level).
- **Relationship to anchor:** Extends the April 15 theme from the Roth question to the broader estimated tax strategy. Positions April 15 as a planning moment, not just a payment deadline.

**LINKEDIN POST 3**

- **Hook line:** "The April 15 deadline has three decisions hidden inside it. Most surgeons see only one."
- **Core insight:** April 15 isn't just the filing deadline. It's simultaneously the Q1 estimated payment deadline, the prior-year IRA contribution deadline, and the implicit start of the extension planning window. Each carries distinct optimization opportunities.
- **Relationship to anchor:** Unifying post that connects the week's content threads — Roth, estimated taxes, and extensions — into a single April 15 framework.

---

**VIDEO PODCAST CONCEPT**

- **Topic and angle:** "The Three April 15 Decisions Hiding in Plain Sight" — walk through the trifecta of decisions embedded in the April 15 deadline.
- **Contrarian hook (first 15 seconds):** "Most surgeons think April 15 is about one thing: taxes. File or extend. Pay what you owe. Move on. But April 15 is actually three separate financial decisions compressed into one day — and most surgeons make only one of them consciously."
- **Key talking points:**
  1. Decision 1: File or extend — and why extending is often the smarter move for surgeons with complex returns
  2. Decision 2: The prior-year IRA contribution — backdoor Roth vs. redirect the money elsewhere
  3. Decision 3: The Q1 estimated payment — are you pacing correctly or flying blind?
  4. How these three decisions interact (e.g., filing an extension changes the Roth contribution timing)
  5. The one-hour April 15 protocol: a checklist for surgeons to run through with their advisor
- **Philosophical close:** "April 15 feels like a deadline. It's actually a decision point. And the quality of your financial life is determined not by the deadlines you meet, but by the decisions you make at each one."

**CLIP CANDIDATES**

1. **The Roth math challenge** (talking point 2): OBBBA made 37% permanent; if you're paying 37% now and expect less later, the Roth may be the most expensive way to save $7,500. Hook: "You've been funding a backdoor Roth every year because someone told you to. The math just changed." ~55s `[BOTH]`
2. **Three decisions, one date** (talking points 1-3): The April 15 trifecta as a rapid-fire reveal. Hook: "April 15 has three decisions hidden inside it. Most surgeons see only one." ~50s `[YT-SHORT]`
3. **The one-hour protocol** (talking point 5): Condensed checklist for surgeons to run through before April 15. Hook: "Here's the one-hour April 15 protocol every surgeon should run with their advisor." ~70s `[IG-REEL]`

---

**FACEBOOK POST 1**

- **Core idea:** "April 15 is two weeks away. Quick reminder: it's not just a filing deadline. It's also the last day to make a prior-year IRA contribution, and the due date for your Q1 estimated tax payment. Three decisions, one date. Don't let the filing question crowd out the other two."
- **Tone adjustment notes:** Practical, brief. This is a genuine reminder wrapped in a strategic reframe. Keep it to 3–4 sentences. Two weeks of lead time gives surgeons enough runway to act on all three decisions.

**FACEBOOK POST 2**

- **Core idea:** "Filed your extension? Good. Now use the next six months. That October 15 window isn't dead time — it's the best planning window you'll get all year."
- **Tone adjustment notes:** Encouraging, normalizing. Reassure extension filers that they made a strategic choice, not a procrastination choice.

---

**Timing rationale:** Two weeks before April 15 — early enough to influence decisions tied to the hard deadline while giving surgeons time to consult their advisors. The Roth piece challenges a reflexive habit at the moment surgeons are beginning to think about whether to fund it. The estimated tax content arrives ahead of the Q1 payment. Maximum relevance with sufficient runway to act.

---

### Week 7: April 6–12

#### Theme: The six-month window — your post-deadline playbook

---

**ANCHOR PIECE**

- **Working title:** "The Six-Month Window Opens April 16: Four Moves to Make Between Now and October 15"
- **Target angle & key argument:** April 15 is this week. Whether the surgeon files or extends, the most valuable planning period of the year is about to start. Most surgeons mentally close the "tax chapter" after April 15 and don't reopen it until October (if they extended) or the following January. Challenge this: the April–October window is when the real optimization happens. Four specific moves that can only be made (or are best started) during this window: (1) S-Corp salary recalibration for the current year, (2) cash balance plan establishment or amendment (requires plan document changes with lead time), (3) Roth conversion analysis (the full-year income picture starts to clarify), and (4) entity restructuring evaluation (with enough runway to implement before year-end).
- **Math example:** A surgeon who filed an extension and uses the six-month window to (1) recalibrate S-Corp salary from $300K to $350K (unlocking $20K in additional retirement contributions), (2) establish a cash balance plan by July 1 (half-year funding of $75K–$150K depending on age), and (3) identify a $40K Roth conversion opportunity in a low-income quarter. Total first-year benefit: $35K–$60K in tax-advantaged value. If the same surgeon had waited until October or later, moves (2) and (3) would have been impossible or severely constrained.
- **Structural approach:** The Checklist/Protocol — open with the imminent April 15 deadline and the mental state that follows ("done with taxes, back to the OR") and challenge it: "You're about to walk out of the most information-rich moment of your financial year — don't waste the next six months." Core Teaching presents the four moves as a protocol with timing deadlines for each: Move 1 by May 15, Move 2 by July 1, Move 3 by September 30, Move 4 by August 31. The protocol format mirrors surgical planning — sequenced, time-bound, and reason-justified.

---

**LINKEDIN POST 1**

- **Hook line:** "April 15 is here. You're filing (or extending). What happens next will matter more. Most surgeons do nothing for the next six months. That's a $60K mistake."
- **Core insight:** The April-to-October window is the most underutilized planning period in a surgeon's financial year. Four specific moves — salary recalibration, cash balance plan setup, Roth analysis, entity restructuring — are best initiated immediately after the deadline.
- **Relationship to anchor:** Direct summary of the anchor's thesis and four-move framework. Creates urgency without the full explanation.

**LINKEDIN POST 2**

- **Hook line:** "If you want a cash balance plan for 2026, the clock started April 16."
- **Core insight:** Cash balance plans require plan document establishment and actuarial certification. Starting the process in April or May gives enough runway for a mid-year implementation. Waiting until Q4 usually means the opportunity is lost for the current tax year.
- **Relationship to anchor:** Zooms into Move 2 (cash balance plan establishment) as a standalone time-sensitive insight. Specific and actionable.

---

**VIDEO PODCAST CONCEPT**

- **Topic and angle:** "What to Do Between Now and October 15 — The Surgeon's Post-Tax-Season Protocol" — a time-sequenced walkthrough of the four planning moves.
- **Contrarian hook (first 15 seconds):** "Tax season is wrapping up. In a few days your CPA will be on a beach somewhere. You'll move on. And that's exactly the problem. The six months between April 15 and October 15 are when the most valuable financial moves of the year get made — or missed. Here's the protocol."
- **Key talking points:**
  1. Why April–October is the real planning season (you have current-year data, time to implement, and no deadline pressure)
  2. Move 1: S-Corp salary recalibration (deadline: May 15 for payroll adjustments)
  3. Move 2: Cash balance plan establishment (deadline: July 1 for meaningful current-year funding)
  4. Move 3: Roth conversion analysis (deadline: September 30 to model before Q4)
  5. Move 4: Entity restructuring evaluation (deadline: August 31 for year-end implementation)
- **Philosophical close:** "In surgery, the preparation before the procedure determines the outcome more than the procedure itself. Your financial life works the same way. The six months of preparation between now and year-end determine whether December is a scramble or a celebration."

**CLIP CANDIDATES**

1. **The $60K mistake** (talking point 1): Most surgeons do nothing for six months after April 15. Hook: "April 15 is here. Most surgeons do nothing for the next six months. That's a $60K mistake." ~50s `[YT-SHORT]`
2. **Cash balance plan clock** (talking point 3): Time-sensitive move with a clear deadline. Hook: "If you want a cash balance plan for 2026, the clock started April 16. Here's why July 1 matters." ~55s `[BOTH]`
3. **The four-move protocol** (talking points 2-5): Rapid overview of all four moves with deadlines. Hook: "Four financial moves, four deadlines, six months. Here's the post-tax-season protocol." ~80s `[IG-REEL]`

---

**FACEBOOK POST**

- **Core idea:** "Whether you file this week or extend, here's the thing most surgeons miss: the next six months are about building, not reacting. One thing to put on your calendar right now: a 30-minute call with your financial advisor to set the planning agenda for April through October. That one call could be worth more than the last three months of tax preparation."
- **Tone adjustment notes:** Encouraging, forward-looking. No urgency language — just an invitational nudge. Frame the call as a simple, low-effort first step.

---

**Timing rationale:** The week leading into April 15. Surgeons are in the final push of tax season — the perfect moment to plant the seed for what comes next. Publishing the six-month playbook the week before the deadline means it lands when surgeons are most receptive to "what now?" messaging. By the time April 15 passes, the protocol is already in their hands.

---

### Week 8: April 13–19

#### Theme: The financial second opinion — are you getting one?

---

**ANCHOR PIECE**

- **Working title:** "The Financial Second Opinion: Why Every Surgeon Should Get One Before Making a Major Move"
- **Target angle & key argument:** Surgeons routinely seek second opinions on complex cases — it's considered good medicine, not a sign of doubt. Yet most surgeons have never sought a second opinion on their financial structure. They have one CPA, one advisor (or none), and one plan that hasn't been stress-tested. Make the case that a financial second opinion — an independent review of your tax strategy, entity structure, retirement plan design, and investment allocation — is the financial equivalent of a second read on a complex imaging study. The value comes from validating the plan, not contradicting the advisor.
- **Math example:** A surgeon whose CPA-recommended strategy includes a 401(k) ($24,500 + $8,000 catch-up = $32,500 total), a standard SEP contribution, and individual investment accounts. A second-opinion review reveals: (1) a cash balance plan could add $150K–$250K in tax-deferred contributions above the 401(k), (2) the SEP conflicts with the 401(k) employer contribution structure and should be restructured, and (3) the individual investment accounts should be rebalanced for tax-loss harvesting coordination. Total value of the second opinion: $50K–$80K in Year 1 tax savings, with compounding benefits over a decade exceeding $400K.
- **Structural approach:** Extended Metaphor — build the entire piece around the "second opinion" concept that surgeons know intimately. Open: "You've ordered the second opinion dozens of times. You've been the second opinion. You know that the value isn't in contradicting the first reading — it's in seeing what the first reader missed." Core Teaching walks through four areas where a second opinion delivers the most value: (1) entity structure, (2) retirement plan design, (3) tax strategy, (4) investment allocation. Close with the tie-back: the best surgeons welcome second opinions because they know the stakes of getting it wrong.

---

**LINKEDIN POST 1**

- **Hook line:** "You've ordered hundreds of second opinions in your career. Have you ever gotten one on your financial plan?"
- **Core insight:** The same discipline that makes you a better surgeon — welcoming independent validation — should apply to your financial structure. A second-opinion review of your entity, retirement plan, and tax strategy typically identifies $50K+ in annual optimization.
- **Relationship to anchor:** Mirrors the anchor's core metaphor in LinkedIn format. The question is the hook; the dollar figure is the proof.

**LINKEDIN POST 2**

- **Hook line:** "Your CPA is great at compliance. Your advisor is great at investments. But who's looking at the whole picture?"
- **Core insight:** Most surgeons have siloed financial professionals — a CPA for taxes, an advisor for investments, a lawyer for estate planning. The gaps between these silos are where the biggest opportunities live. A coordinated financial second opinion looks at how these pieces interact.
- **Relationship to anchor:** Extends the anchor's argument from "get a second opinion" to "get a coordinated one." Addresses the common objection that the surgeon already has advisors.

---

**VIDEO PODCAST CONCEPT**

- **Topic and angle:** "Why I Recommend Every Surgeon Get a Financial Second Opinion — and What to Look For" — conversational, advisory tone.
- **Contrarian hook (first 15 seconds):** "I'm about to say something that might sound self-serving coming from a financial advisor: I think every surgeon should get an independent second opinion on their financial plan. Not because your current advisor is bad. But because the cost of not getting one — at your income level — is too high to leave to assumption."
- **Key talking points:**
  1. The surgical second opinion parallel — why it's good medicine and good finance
  2. The four areas a second opinion should cover (entity, retirement, tax, investments)
  3. What a good second opinion process looks like (and what it doesn't — it's not a sales pitch)
  4. The "silo gap" problem — how uncoordinated advisors create structural leaks
  5. How to evaluate a second opinion: what questions to ask and what red flags to watch for
- **Philosophical close:** "The best surgeons I know aren't threatened by a second opinion. They're strengthened by it. The same should be true of your financial plan. If your strategy is sound, a second opinion confirms it. If it's not — you'd rather know now than in ten years."

**CLIP CANDIDATES**

1. **The silo gap** (talking point 4): Your CPA, advisor, and lawyer each see one piece. The gaps between them are where the money leaks. Hook: "Your CPA is great at compliance. Your advisor is great at investments. But who's looking at the whole picture?" ~55s `[BOTH]`
2. **The surgical parallel** (talking point 1): The second opinion as good medicine and good finance. Hook: "You've ordered hundreds of second opinions in your career. Have you ever gotten one on your financial plan?" ~50s `[YT-SHORT]`
3. **What to look for** (talking point 5): Red flags and questions for evaluating a second opinion. Hook: "If someone offers you a financial second opinion, here are the three questions to ask before you sit down." ~65s `[IG-REEL]`

---

**FACEBOOK POST**

- **Core idea:** "Here's something I've noticed: surgeons are some of the most disciplined professionals in the world when it comes to seeking peer review and second opinions in clinical practice. But when it comes to their finances, most have a plan that was set up years ago and has never been independently reviewed. This isn't a criticism — it's an observation about where the same rigor you bring to the OR could transform your financial life. If nothing else, asking 'has anyone else looked at this?' is free."
- **Tone adjustment notes:** Observational, warm. Not a pitch for services — genuinely reflective on the disconnect between clinical rigor and financial passivity. Invitational close.

---

**Timing rationale:** The week of April 15 and just after — the tax-deadline urgency is resolving, and there's breathing room for a broader strategic message. The "second opinion" piece transitions naturally from April's tax-focused content to May's practice transition theme. Many surgeons are in a reflective post-tax-season mindset — the ideal moment to introduce the idea of stepping back and examining the full picture.

---

## MAY 2026: The Exit Conversation

> **Surgeon context:** Medical conference season begins (AAOS, regional meetings). Surgeons are exposed to practice transition conversations at industry events — PE firms are visible, colleagues are discussing deals, and the idea of "what's next" is in the air. Capitalize on this natural priming. Content should publish early May to land before and during conference season.

---

### Week 9: April 20–26

#### Theme: The 5-year exit timeline starts now

---

**ANCHOR PIECE**

- **Working title:** "The 5-Year Exit Timeline: What to Start Doing Now If You Want Options Later"
- **Target angle & key argument:** Most surgeons think about practice transition 12–18 months before they want to exit. That's too late to optimize the financial outcome. Map out the 5-year timeline with specific milestones: Year 5 (valuation baseline + entity restructuring), Year 4 (personal goodwill documentation + revenue source diversification), Year 3 (associate recruitment + systematization to reduce surgeon-dependence), Year 2 (buyer identification + earnout negotiation framework), Year 1 (execution, closing, and wealth transition planning). The reframe: an exit isn't a single event — it's a 5-year treatment plan, and starting early is the difference between choosing your outcome and settling for whatever's available.
- **Math example:** A 55-year-old surgeon with a practice valued at $2.8M at current operations. With a 5-year timeline: Year 5 restructuring from S-Corp to optimized entity adds $200K in enterprise value by separating personal goodwill. Year 4 revenue diversification (adding an ASC ownership stake, building referral network) increases practice value to $3.4M. Year 3 associate hire reduces surgeon-dependence, making the practice transferable and boosting the multiple from 3x to 4x EBITDA. Year 2 buyer identification in a strong PE market yields competitive bids. Result: a $4.2M exit at age 60 vs. a $2.8M distressed-timeline sale at 60 — a $1.4M difference, driven entirely by starting five years early instead of two.
- **Structural approach:** The Protocol/Timeline — the 5-year plan mirrors a long-term surgical treatment protocol. Open with: "If a patient came to you with a degenerative condition, you wouldn't wait until they couldn't walk to start treatment. You'd build a multi-year plan. Your practice exit deserves the same approach." Core Teaching walks through each year's milestones as a protocol phase. Math example shows the cumulative value creation at each phase. Close: "The surgeons who exit well didn't get lucky. They started early."

---

**LINKEDIN POST 1**

- **Hook line:** "Most surgeons start thinking about their practice exit 18 months before they want to leave. That's 3.5 years too late."
- **Core insight:** A 5-year exit timeline creates $1M+ in additional value through entity restructuring, personal goodwill documentation, and strategic buyer positioning — none of which can be done in the final 18 months.
- **Relationship to anchor:** Compresses the anchor's $1.4M value gap into a single provocation. The 18-months-too-late framing creates urgency.

**LINKEDIN POST 2**

- **Hook line:** "Your practice exit isn't a transaction. It's a treatment plan."
- **Core insight:** Like a complex surgical case, a practice exit requires staging — diagnosis (valuation), pre-treatment (restructuring), intervention (buyer engagement), and recovery (wealth transition). Compressing this into 12 months is the financial equivalent of skipping the treatment plan and going straight to the OR.
- **Relationship to anchor:** Uses the surgical metaphor from the anchor to create a memorable framework for the exit planning concept.

**LINKEDIN POST 3**

- **Hook line:** "Conference season is here. You'll hear more about practice transitions in the next 30 days than in the last 12 months. Here's what to filter for."
- **Core insight:** Three questions to ask in any conference conversation about practice transition: (1) What was the timeline from first conversation to close? (2) How was personal goodwill handled? (3) What does the earnout structure look like 3 years out? These cut through the pitch decks and get to the structural reality.
- **Relationship to anchor:** Bridges the anchor's planning message to the immediate context of conference season. Positions Jared as the advisor who prepares you before you get pitched.

---

**VIDEO PODCAST CONCEPT**

- **Topic and angle:** "Conference Season Is Here — and PE Firms Are Buying Dinner. What to Know Before You Go." — a pre-conference briefing on the practice transition landscape.
- **Contrarian hook (first 15 seconds):** "You're about to walk into a conference where at least three private equity firms will try to buy you dinner. They'll have beautiful pitch decks, compelling multiples, and a vision for 'partnership.' I'm not here to tell you they're wrong. I'm here to tell you what they won't mention — and what you need to know before the appetizers arrive."
- **Key talking points:**
  1. The current PE landscape in orthopedics — second wave, major players, deal structures
  2. What the 5-year exit timeline looks like (and why it matters even if you're not selling to PE)
  3. The three questions to ask any acquirer: timeline, goodwill treatment, earnout structure
  4. How to distinguish a genuine partnership from a financial extraction
  5. What "options later" actually means — why starting the planning process gives you leverage, not commitment
- **Philosophical close:** "The surgeons who get the best outcomes in practice transitions aren't the ones with the best negotiators. They're the ones who started planning early enough to have choices. Conference season is a wonderful time to start thinking. Just make sure you're thinking on your timeline, not someone else's."

**CLIP CANDIDATES**

1. **The $1.4M exit gap** (talking point 2): 5-year timeline exit at $4.2M vs. distressed 2-year exit at $2.8M. Hook: "The difference between a $4.2M exit and a $2.8M exit is three years of planning." ~55s `[BOTH]`
2. **Three questions for any acquirer** (talking point 3): Timeline, goodwill, earnout. Hook: "Conference season is here. Before you sit down with any PE firm, ask these three questions." ~50s `[YT-SHORT]`
3. **Partnership vs. extraction** (talking point 4): How to tell the difference. Hook: "They'll call it a partnership. Here's how to tell if it's actually a financial extraction." ~60s `[IG-REEL]`

---

**FACEBOOK POST**

- **Core idea:** "Conference season is kicking off, and if you're in orthopedics, practice transition will be the conversation in every hallway and at every dinner. My advice: listen, learn, and take notes — but don't make any decisions at the conference. The best practice exits are planned over years, not decided over dinner. Give yourself a 5-year runway, and you'll have choices. Rush it, and you'll have regrets."
- **Tone adjustment notes:** Warm, protective. This is the "wise friend" tone — not dismissive of the conference excitement, but grounding it in longer-term perspective.

---

**Timing rationale:** Late April — just before the major conference season. Surgeons are booking travel, reviewing agendas, and mentally shifting from clinical work to industry conversations. This is the moment to set the strategic frame before PE firms and colleagues set it for them.

---

### Week 10: April 27 – May 3

#### Theme: Concentration risk — your practice is your biggest bet

---

**ANCHOR PIECE**

- **Working title:** "Private Equity Is Calling. Here's What They're Not Telling You Over Dinner."
- **Target angle & key argument:** PE consolidation in orthopedics is in its second wave. Surgeons are getting approached with compelling pitch decks and promises of "liquidity events." Cut through the pitch: explain rollover equity (typically 30–35% of the sale price must be reinvested in the acquiring entity), compensation scrapes (post-acquisition, surgeon compensation often declines 15–25% through productivity thresholds and management fees), EBITDA engineering (buyers normalize earnings in ways that may not reflect the practice's true run rate), and what the 3–5 year hold period actually means for the selling surgeon (you're now an employee of the entity that bought your life's work). The tone is not anti-PE — it's pro-clarity. Some PE deals are excellent. The surgeon just needs to understand what they're agreeing to.
- **Math example:** A surgeon sells a practice valued at $4M. PE deal structure: $2.6M cash at close + $1.4M in rollover equity (35%). The $2.6M cash is subject to capital gains tax — but only if personal goodwill was established pre-sale. Without personal goodwill documentation, the full $4M may be taxed as ordinary income (37% + 3.8% NIIT) vs. long-term capital gains (20% + 3.8%). Tax difference: ordinary income treatment = $1.63M in tax; capital gains treatment = $952K in tax. The $678K difference is entirely a function of pre-transaction planning — not the deal terms. On the rollover equity: if the second "bite of the apple" happens in 4 years at a 2.5x return, the $1.4M becomes $3.5M — but the surgeon has no control over timing, terms, or outcome. That's concentrated, illiquid risk in someone else's hands.
- **Structural approach:** Contrarian Reframe — open with the seductive PE pitch: "the deck was beautiful, the multiple was compelling, and the promise of 'partnership' felt right." Then reframe: "Here's what the deck didn't include." Core Teaching walks through four elements PE pitch decks de-emphasize: rollover equity terms, compensation scrapes, EBITDA normalization, and hold period dynamics. The math example on personal goodwill vs. ordinary income treatment provides the "this is what it costs you" anchor. Close: "I'm not anti-PE. I'm anti-surprise. The best deals happen when the surgeon walks in with the same level of preparation they bring to the OR."

---

**LINKEDIN POST 1**

- **Hook line:** "A surgeon showed me a PE pitch deck last week. It was beautiful. It was also missing four things."
- **Core insight:** PE pitch decks emphasize the multiple and the vision. They de-emphasize rollover equity terms, compensation scrapes, EBITDA normalization methods, and hold period dynamics. Understanding all four is the difference between a good deal and an expensive lesson.
- **Relationship to anchor:** Teases the anchor's four missing elements without full explanation. The "what's missing" framing drives curiosity.

**LINKEDIN POST 2**

- **Hook line:** "Valuation day is coming. Here are three numbers your practice appraiser won't emphasize — but your wealth depends on."
- **Core insight:** The three numbers that matter most in a practice valuation aren't the headline multiple: (1) personal goodwill vs. enterprise goodwill split (determines tax treatment of the sale), (2) normalized EBITDA (what buyers actually use, not your accountant's version), and (3) the working capital adjustment (the cash you think you're keeping but aren't).
- **Relationship to anchor:** Extends the anchor's "what they don't tell you" theme from PE pitch decks to the valuation process itself. Conference season is when valuation conversations start.

**LINKEDIN POST 3**

- **Hook line:** "Your practice is 46% of your net worth. You'd never put 46% of your portfolio in a single stock. So why is your biggest asset the one you've never diversified?"
- **Core insight:** An orthopedic surgeon with a $3M practice valuation, a $1.5M home, and a $2M portfolio has 46% of their net worth in a single illiquid asset they can't sell in pieces. Concentration risk in a practice is real — and the de-risking strategy starts years before the exit.
- **Relationship to anchor:** Bridges from the PE-specific content to the broader concentration risk theme. Positions practice diversification as a strategic priority regardless of whether the surgeon is considering a PE deal.

---

**VIDEO PODCAST CONCEPT**

- **Topic and angle:** "Your Practice Is Your Biggest Bet. Are You Treating It Like One?" — the editorial plan's designated YouTube piece for May.
- **Contrarian hook (first 15 seconds):** "If I told you that 46% of your net worth was concentrated in a single, illiquid asset that you can't sell in pieces, can't hedge, and can't diversify — you'd call that reckless. But that's exactly what your practice ownership represents. And almost nobody is talking about it that way."
- **Key talking points:**
  1. The concentration math — how to calculate what percentage of your net worth your practice represents
  2. Why "my practice IS my retirement plan" is the most dangerous sentence in surgeon finance
  3. The three de-risking strategies: diversify revenue sources, establish personal goodwill documentation, and build a transition timeline
  4. PE deals as one form of de-risking — but not the only one, and not always the best
  5. How to evaluate your practice as an investment: what return are you earning on the time and capital invested?
- **Philosophical close:** "You built your practice with precision, skill, and an enormous investment of your best years. It deserves to be treated with the same financial rigor you'd apply to any asset of its size. The question isn't whether you'll exit eventually — you will. The question is whether the exit will be on your terms."

**CLIP CANDIDATES**

1. **The 46% concentration** (talking point 1): The net worth math delivered cold. Hook: "46% of your net worth is in a single illiquid asset you can't sell in pieces. You'd call that reckless in any portfolio." ~50s `[BOTH]`
2. **The most dangerous sentence** (talking point 2): "My practice IS my retirement plan." Hook: "There's one sentence I hear from surgeons that tells me everything about their financial risk." ~55s `[YT-SHORT]`
3. **The $678K tax difference** (from anchor context): Personal goodwill documentation turns ordinary income into capital gains. Hook: "The difference between ordinary income and capital gains on a $4M practice sale? $678,000 in taxes." ~55s `[BOTH]`
4. **Rollover equity reality** (from anchor context): What PE pitch decks don't mention. Hook: "A surgeon showed me a PE pitch deck last week. It was beautiful. It was also missing four things." ~70s `[IG-REEL]`

---

**FACEBOOK POST**

- **Core idea:** "Conference hallway conversations this week: 'So-and-so sold to PE and got a great multiple.' What they don't mention: the rollover equity they're still holding, the compensation adjustment post-close, and the fact that they're now an employee of the entity that bought their practice. PE deals can be great. But the details matter more than the headline number. Before you fall in love with a multiple, make sure you understand what you're actually getting — and giving up."
- **Tone adjustment notes:** Conversational, protective. Acknowledge the excitement without being dismissive. The goal is to add nuance to the conference buzz, not dampen it.

---

**Timing rationale:** Late April into early May, during the peak of early conference season. PE conversations are happening in real time. This content meets surgeons where the conversation already is — and provides the framework they need before (not after) a deal gets serious.

---

### Week 11: May 4–10

#### Theme: Personal goodwill — the tax strategy hidden inside your reputation

---

**ANCHOR PIECE**

- **Working title:** "Personal Goodwill: The Tax Strategy That Could Save You $300K+ on Your Practice Sale"
- **Target angle & key argument:** For surgeons 3–5 years from exit, personal goodwill planning must start now — not during sale negotiations. Personal goodwill is the value attributable to the surgeon's individual reputation, skills, referral relationships, and patient loyalty. It's distinct from enterprise goodwill (the practice's brand, systems, and transferable value). The tax impact is massive: personal goodwill is taxed as long-term capital gains (20% + 3.8% NIIT) while enterprise goodwill allocated to certain assets can be taxed at ordinary income rates (37% + 3.8%). But — and this is critical — personal goodwill must be established and documented before negotiations begin, not asserted during them. The IRS scrutinizes after-the-fact personal goodwill claims. The time to start is now.
- **Math example:** A $4M practice sale. Without personal goodwill documentation: the entire amount is allocated to enterprise assets and taxed at a blended rate averaging 32% (mix of ordinary income on some asset classes and capital gains on others) = $1.28M in tax. The surgeon keeps $2.72M. With proper personal goodwill documentation allocating 40% ($1.6M) to personal goodwill: the $1.6M is taxed at 23.8% (LTCG + NIIT) = $381K, and the remaining $2.4M is taxed at the blended 32% = $768K. Total tax: $1.149M. The surgeon keeps $2.851M. Difference: $131K. Now extend the personal goodwill allocation to 55% (achievable with strong documentation): personal goodwill of $2.2M taxed at 23.8% = $524K, remaining $1.8M at 32% = $576K. Total tax: $1.1M. Surgeon keeps $2.9M — saving $180K+ compared to zero personal goodwill planning. Over the full range of realistic allocations, the savings typically fall between $130K and $350K.
- **Structural approach:** The Diagnostic Framework — open with: "If you're 3–5 years from selling your practice, there's a diagnosis you need to make now — not later. And the test isn't an MRI. It's a personal goodwill assessment." Core Teaching explains: what personal goodwill is (and isn't), why the IRS cares about the distinction, how to document it before a sale (non-compete agreements, patient referral documentation, personal branding evidence), and the timeline for proper establishment. Math example shows the tax savings at different allocation percentages. Close: "Personal goodwill is the value of everything you've built that walks out the door when you do. Make sure it's documented — because undocumented goodwill is worthless in a tax court."

---

**LINKEDIN POST 1**

- **Hook line:** "The difference between paying 23.8% and 37% on your practice sale comes down to one thing: documentation you should have started two years ago."
- **Core insight:** Personal goodwill — taxed at capital gains rates — must be documented before sale negotiations begin. After-the-fact claims get scrutinized and often denied by the IRS.
- **Relationship to anchor:** Crystallizes the anchor's core message: the documentation must come first, not the sale.

**LINKEDIN POST 2**

- **Hook line:** "Your reputation, your referral network, your surgical skill — they have a dollar value. And if you don't document that value before you sell, the IRS will tax it like a piece of equipment."
- **Core insight:** Personal goodwill represents the surgeon's individual value that doesn't transfer without the surgeon. If it isn't separated from enterprise goodwill before the sale, the entire practice value may be taxed at higher ordinary income rates on certain asset allocations.
- **Relationship to anchor:** Emotional reframe — makes "personal goodwill" feel personal by connecting it to the surgeon's identity and work, not just a tax category.

---

**VIDEO PODCAST CONCEPT**

- **Topic and angle:** "The Hidden Tax Bill in Every Practice Sale — and How to Reduce It by $300K" — visual walkthrough of the personal goodwill allocation and its tax impact.
- **Contrarian hook (first 15 seconds):** "When you sell your practice, the IRS is going to look at the total price and ask one question: how much of this was the surgeon, and how much was the business? Your answer to that question — backed by documentation — could mean the difference between a $950K tax bill and a $1.28M tax bill. That $330K gap is entirely within your control. But only if you start now."
- **Key talking points:**
  1. What personal goodwill is — your reputation, referral network, skills, patient relationships
  2. Why it matters for taxes — capital gains (23.8%) vs. ordinary income (up to 40.8%) on certain allocations
  3. How to document it — non-compete agreements, referral source analysis, patient loyalty metrics
  4. The timeline — 2–3 years minimum before a sale for credible documentation
  5. The IRS perspective — why after-the-fact claims fail and how to bulletproof yours
- **Philosophical close:** "You've spent decades building a reputation that patients trust and colleagues respect. That reputation has a value — a significant, measurable, taxable value. The question is whether you'll document it proactively, or lose hundreds of thousands of dollars because nobody told you to."

**CLIP CANDIDATES**

1. **The $330K gap** (contrarian hook): The tax difference between documenting personal goodwill and not. Hook: "When you sell your practice, the IRS asks one question. Your answer could save you $330K." ~50s `[BOTH]`
2. **Capital gains vs. ordinary income** (talking point 2): The rate comparison on practice sale proceeds. Hook: "23.8% or 40.8%. The tax rate on your practice sale depends on one thing you should have documented two years ago." ~55s `[YT-SHORT]`
3. **How to document personal goodwill** (talking point 3): The three documentation components. Hook: "Three things you need to document now if you plan to sell your practice in the next five years." ~70s `[IG-REEL]`

---

**FACEBOOK POST**

- **Core idea:** "Something I wish more surgeons knew earlier: when you sell your practice, there's a massive tax difference between the value attributed to 'you' (your skills, reputation, relationships) and the value attributed to 'the business' (equipment, systems, brand). The 'you' part can be taxed at capital gains rates. The 'business' part may not be. But you have to document the distinction years before you sell. Not months. Years."
- **Tone adjustment notes:** Educational, warm. Frame it as a "wish you knew" insight rather than a "you need to act" command. The goal is awareness, not urgency.

---

**Timing rationale:** Early May, during active conference season. Surgeons are having practice transition conversations. Personal goodwill is one of the most valuable and least understood aspects of practice exit planning. Timing this piece when surgeons are already thinking about transitions maximizes the chance they'll act on it.

---

### Week 12: May 11–17

#### Theme: What "normalized EBITDA" really means — and why it matters for your exit

---

**ANCHOR PIECE**

- **Working title:** "What Buyers Actually See When They Look at Your Practice Financials"
- **Target angle & key argument:** Surgeons are used to looking at their practice financials through the lens of collections, overhead ratios, and net income. Buyers — whether PE firms, hospital systems, or individual acquirers — look at a completely different set of numbers: normalized EBITDA. Normalization strips out owner-specific expenses (the surgeon's above-market salary, personal vehicles on the business, family members on payroll, one-time expenses), and what remains is the practice's transferable earning power. Most surgeons are shocked by the gap between their reported net income and their normalized EBITDA — and that gap determines valuation. Walk through the most common normalization adjustments and what they mean for the seller.
- **Math example:** A practice with reported net income of $1.1M. The surgeon pays herself $800K (above-market by $150K — market rate for a replacement surgeon is $650K). A family member is on payroll at $120K for a part-time administrative role (market rate: $55K). The practice leases a luxury vehicle ($1,800/month, $21,600/year) through the business. One-time legal expenses of $45K last year. Normalized EBITDA: $1.1M (net income) + $150K (salary adjustment) + $65K (family member adjustment) + $21.6K (vehicle) + $45K (one-time expense) = $1.38M. At a 4x multiple, the practice is valued at $5.5M on normalized EBITDA vs. $4.4M on reported net income — a $1.1M valuation difference. Understanding normalization isn't just accounting. It's wealth.
- **Structural approach:** Math-First Approach — open with the normalization math as a "reveal" moment: "A surgeon told me her practice was worth $4.4M. After normalization, it was $5.5M. Neither number was wrong — they were just measuring different things." Core Teaching walks through the five most common normalization adjustments for surgeon practices (owner compensation, family employment, personal expenses through the business, one-time costs, and below-market lease terms). Each adjustment includes the typical dollar range for orthopedic practices. Close: "Buyers will normalize your financials whether you understand the process or not. The question is whether you'll be ready to tell the story those normalized numbers should tell."

---

**LINKEDIN POST 1**

- **Hook line:** "Your practice's reported net income and its value to a buyer are two very different numbers. The gap between them is called normalized EBITDA — and most surgeons have never calculated it."
- **Core insight:** Normalization adjustments — owner salary, family employment, personal expenses — can shift practice valuation by $500K–$1.5M at typical orthopedic multiples. Understanding these adjustments before a buyer presents them gives the surgeon control over the narrative.
- **Relationship to anchor:** Distills the anchor's core insight into the gap between reported income and buyer-facing value.

**LINKEDIN POST 2**

- **Hook line:** "If your spouse is on your practice payroll, a buyer is already adjusting for it. Here's what that means for your valuation."
- **Core insight:** Family member employment, above-market owner compensation, and personal expenses through the business are the three most common normalization adjustments. They're not problems — but they need to be understood and strategically managed before a sale process begins.
- **Relationship to anchor:** Zooms into one specific, relatable normalization adjustment (family payroll) as a hook into the broader concept.

---

**VIDEO PODCAST CONCEPT**

- **Topic and angle:** "Practice Valuation 101: What Buyers See That You Don't" — an accessible explanation of EBITDA normalization for surgeon practice owners.
- **Contrarian hook (first 15 seconds):** "If someone offered to buy your practice tomorrow, you'd probably quote a number based on your revenue or net income. The buyer would quote a very different number — based on something called normalized EBITDA. And the gap between your number and their number could be more than a million dollars in either direction. Let me explain what you're missing."
- **Key talking points:**
  1. What EBITDA is and why buyers use it instead of net income
  2. The five normalization adjustments and why they matter
  3. How to calculate your own normalized EBITDA in 30 minutes
  4. Why normalization can work for you (not just against you)
  5. The strategic preparation: cleaning up your financials 2–3 years before a sale
- **Philosophical close:** "Knowledge is leverage. When you understand how buyers evaluate your practice, you stop being a passive seller and start being a strategic one. And that shift — from passive to strategic — is the difference between accepting a number and negotiating from a position of strength."

**CLIP CANDIDATES**

1. **The $1.1M valuation gap** (talking points 1-2): Net income says $4.4M, normalized EBITDA says $5.5M. Hook: "Your practice is worth $4.4M. Or is it $5.5M? The answer depends on one concept most surgeons have never heard of." ~55s `[BOTH]`
2. **The family payroll adjustment** (talking point 2): A relatable normalization example. Hook: "If your spouse is on your practice payroll, a buyer is already adjusting for it. Here's what that means for your valuation." ~50s `[YT-SHORT]`
3. **30-minute EBITDA calculation** (talking point 3): How to calculate your own number. Hook: "You can calculate your practice's normalized EBITDA in 30 minutes. Here's how." ~75s `[IG-REEL]`

---

**FACEBOOK POST**

- **Core idea:** "Quick note for any surgeon thinking about selling their practice in the next few years: the number your accountant shows you and the number a buyer sees are probably very different. Understanding why — and what 'normalized EBITDA' actually means — is the single most valuable thing you can learn before any conversation with a potential buyer. Not investment advice. Just information that's worth having."
- **Tone adjustment notes:** Low-key, informational. Frame it as a helpful note, not a pitch. "Quick note" and "just information" keep the tone casual.

---

**Timing rationale:** Mid-May — conference season is in full swing, and many surgeons have had their first conversations with PE firms or colleagues about practice transitions. This is the "now that you're interested, here's what you need to understand" piece. It builds on the PE content from Week 10 and the personal goodwill content from Week 11 to complete the valuation literacy arc.

---

### Week 13: May 18–24

#### Theme: Designing the next chapter — from exit planning to life planning

---

**ANCHOR PIECE**

- **Working title:** "Stop Planning Your Exit. Start Planning Your Next Chapter."
- **Target angle & key argument:** The last four weeks have been about practice transition mechanics — timelines, PE deals, personal goodwill, valuation. This week pivots from the mechanics to the meaning. The most common regret among surgeons who sell their practice centers on not having a plan for what comes after. The identity loss of leaving the OR is real, documented, and more financially damaging than any tax mistake. Surgeons who exit without a "next chapter" plan tend to make poor financial decisions in the first 18 months post-exit: overspending, impulsive investments, lifestyle inflation. The argument: your financial exit plan is only as good as your life plan. Design the next chapter with the same intentionality you brought to the last one.
- **Math example:** The financial cost of an unplanned transition. A surgeon who sells a practice for $4M, pays $1.1M in taxes, and nets $2.9M. Without a structured post-exit plan, the typical pattern: lifestyle spending increases 30% in Year 1 (from $350K to $455K), a $500K "investment" in a friend's business that fails, and a portfolio drawdown of $200K to cover the gap. By Year 3, the $2.9M has become $1.7M — a 41% decline with no practice income to replenish it. Compare: the same surgeon with a structured transition plan including a defined annual spending budget ($375K), a systematic investment strategy, and a purposeful "next chapter" that generates meaning (and possibly income). By Year 3, the $2.9M has grown to $3.2M while funding a $375K/year lifestyle. The difference comes down to having a plan for the life, not just the money.
- **Structural approach:** Philosophical Anchor — this is the contemplative, big-picture piece that closes the May arc. Open with: "You've spent the last month thinking about valuations, deal terms, and tax strategies. Good. Now let me ask you a harder question: what are you transitioning to?" Core Teaching covers three elements of a "next chapter" plan: (1) identity work (who are you when you're not a surgeon?), (2) purpose architecture (what gives your days structure and meaning?), and (3) financial guardrails (how do you prevent the emotional decisions that erode exit wealth?). Close with the tie-back to Jared's core belief: wealth is control, and control means having a plan — for your money and for your life.

---

**LINKEDIN POST 1**

- **Hook line:** "The most expensive mistake in practice transition is walking away without a plan for what comes after."
- **Core insight:** Surgeons who exit without a structured "next chapter" plan lose an average of 30–40% of their exit wealth within three years — not to bad investments, but to the unplanned spending and impulsive decisions that come from a life without structure.
- **Relationship to anchor:** Leads with the financial consequence to hook the LinkedIn audience, then redirects to the life-planning message.

**LINKEDIN POST 2**

- **Hook line:** "Who are you when you're not a surgeon?"
- **Core insight:** The identity question is the hardest part of practice transition — and the one most financial advisors never address. Surgeons who answer it before they exit make better financial decisions, maintain healthier spending patterns, and report higher life satisfaction post-transition.
- **Relationship to anchor:** The philosophical heart of the anchor, stripped to a single question. Provocative, brief, and shareable.

**LINKEDIN POST 3**

- **Hook line:** "This month we talked about PE deals, personal goodwill, and practice valuations. Here's what matters more than all of it."
- **Core insight:** The mechanics of a practice exit are important. The purpose behind the exit is more important. A $4M exit that leads to purposeless spending and identity loss isn't a success — it's a well-compensated failure. Design the life first, then optimize the transaction.
- **Relationship to anchor:** Month-closing synthesis post that ties the four weeks of May content into a single philosophical point. Bookends the month's arc.

---

**VIDEO PODCAST CONCEPT**

- **Topic and angle:** "Life Beyond the OR: What the Best Practice Transitions Have in Common" — a reflective, conversational piece on what makes a successful post-practice life.
- **Contrarian hook (first 15 seconds):** "I've worked with dozens of surgeons through practice transitions. The ones who are happiest two years later aren't the ones who got the highest multiple. They're the ones who knew what they were transitioning to — not just what they were leaving. This video is about the difference between an exit and a transition."
- **Key talking points:**
  1. The identity gap — why leaving the OR feels like losing a part of yourself, and how to prepare for it
  2. The spending trap — post-exit lifestyle inflation is the most predictable and preventable wealth destroyer
  3. The purpose question — surgeons who find a "next chapter" purpose (teaching, consulting, philanthropy, new ventures) maintain better financial discipline
  4. The financial guardrail system — structured spending, systematic investing, and quarterly check-ins as the antidote to impulsive post-exit decisions
  5. Two case studies (anonymized): one surgeon who planned, one who didn't — and where they stood three years later
- **Philosophical close:** "Retirement is a financial construct. Transition is a human one. The financial plan gets you out of the OR. The life plan determines whether what comes next is your best chapter or just the one that came last. Design it with the same precision you brought to everything else."

**CLIP CANDIDATES**

1. **The identity question** (talking point 1): The hardest question in practice transition. Hook: "Who are you when you're not a surgeon? That question is worth more than any valuation." ~45s `[YT-SHORT]`
2. **The spending trap** (talking point 2): $2.9M becomes $1.7M in three years without a plan. Hook: "A surgeon netted $2.9M from a practice sale. Three years later, $1.7M was left. Here's what happened." ~55s `[BOTH]`
3. **The planned vs. unplanned comparison** (talking point 5): Two surgeons, same exit, different outcomes. Hook: "Two surgeons. Same exit value. One planned, one didn't. Here's where they stood three years later." ~75s `[IG-REEL]`
4. **The philosophical close** (philosophical close): "Retirement is a financial construct. Transition is a human one." Hook: "Stop planning your retirement. Start planning your transition." ~45s `[BOTH]`

---

**FACEBOOK POST 1**

- **Core idea:** "I want to share something personal. The happiest clients I work with aren't the ones with the most money. They're the ones who know what their money is for. When a surgeon approaches practice transition with a clear vision of what comes next — not just financially, but personally — everything changes. The financial decisions get easier. The spending stays disciplined. And the life that unfolds isn't an afterthought — it's by design."
- **Tone adjustment notes:** Personal, warm, reflective. This is the emotional close to the month. Vulnerability and warmth over data and frameworks.

**FACEBOOK POST 2**

- **Core idea:** Quarter wrap: "Three months of content, one message: the foundation of your financial life is your structure, the system that turns your income, your tax strategy, and your practice valuation into the life you actually want to live. If anything from the last three months resonated, the best next step is the simplest one: a single conversation with someone who can look at your full financial picture and tell you where the gaps are."
- **Tone adjustment notes:** Invitational, not salesy. This is the quarter's closing handshake — warm, reflective, and forward-looking. The "single conversation" CTA is soft and natural.

---

**Timing rationale:** Mid-to-late May — closing the quarter's content arc. After four weeks of practice transition mechanics, this philosophical close provides the "why" behind all the "how." It's also a natural transition point: June shifts to mid-year tax check, and this piece sets up that pivot by moving from "what's your exit plan?" to "what's your life plan?" The reflective tone matches the end-of-conference-season wind-down.

---

## Content Flow Summary

The 13-week arc follows a deliberate narrative progression:

**March (Weeks 1–4): Diagnosis**
The structure problem → S-Corp salary → reimbursement shift → effective hourly rate. Each week zooms deeper into the structural gaps that determine how much a surgeon actually keeps. The month builds from the broad thesis ("you're understructured") to the specific diagnostic number ("here's your effective hourly rate").

**April (Weeks 5–8): Interpretation**
Tax return as diagnostic → April 15 trifecta → the 6-month window → the financial second opinion. Each week takes the diagnostic findings from March and shows surgeons how to read them, what to do about them, and who to involve. The month builds from "here's what the data says" to "here's the plan."

**May (Weeks 9–13): Vision**
5-year exit timeline → PE deals and concentration risk → personal goodwill → valuation literacy → life beyond the OR. Each week expands the lens from tactical planning to strategic positioning to life design. The month builds from "what's the timeline?" to "what's the purpose?"

The entire quarter follows a single trajectory: **from diagnosis to treatment plan to desired outcome** — the same three-phase arc that defines every surgical case.

---

## Production Notes

### Channel Cadence

| Channel | Recommended Publish Day | Notes |
|---------|------------------------|-------|
| Anchor (blog) | Monday or Tuesday | Start the week's theme; allow social content to build on it |
| LinkedIn | Tuesday, Wednesday, Thursday | One per day during business hours; space for engagement |
| Video podcast (YouTube) | Thursday or Friday | Midweek depth piece; weekend viewing window |
| Short-form clips (YouTube Shorts) | Friday, Saturday, or Sunday | Stagger after full episode; space clips across 2-3 days |
| Short-form clips (Instagram Reels) | Friday, Saturday, or Sunday | Can post same day as YouTube Shorts or offset by a day |
| Facebook | Wednesday and/or Saturday | Midweek for conversational pieces; Saturday for reflective |

### Building Block Assembly Reminders

Per the Content Recipe:
- **Blog posts:** All 8 blocks (Research → Brief → Draft → Relevance Validation → Voice Validation → Edit/Polish → Distribution → Performance)
- **LinkedIn posts:** Blocks 1, 3, 4, 5, 7, 8 (lighter brief, shorter draft)
- **Video podcast scripts:** All 8 blocks (full sequence, script format) + Clip Extraction Map
- **Short-form clips:** Blocks 7, 8 only (Distribution + Performance; clips derive from validated podcast)
- **Facebook posts:** Blocks 1, 3, 5, 7, 8 (lightest touch, adapted from LinkedIn or blog)

### Voice Validation Reminders

Before publishing any piece, confirm:
- [ ] Opens with a story, conversation, or specific scenario (not a definition)
- [ ] Contains at least one contrarian reframe or myth-bust
- [ ] Uses direct "you" address throughout
- [ ] Includes a concrete numerical example at surgeon-level income ($700K–$2M)
- [ ] Reads like a conversation with a trusted advisor, not a brochure
- [ ] Ends with "Capably Yours, Jared" (blog/email) or a clear takeaway (social)
- [ ] If it sounds like marketing, rewrite it. If it sounds like a thoughtful professional explaining a decision, it's correct.

### Dynamic Research Checkpoints

Per the Content Calendar Layer 2:
- **Monthly scan due March 5, April 5, May 5** — check for Fed decisions, legislation, market events, CMS updates, PE deal activity
- **Quarterly deep scan due April 1** — comprehensive review including practice valuation trends, competitive scan, content performance data
- **Event-triggered scans** — activated immediately if: FOMC makes unexpected move, major tax bill advances, significant PE deal announced in orthopedics

### Relevance Flags to Monitor

| Flag | Timing | Action If Triggered |
|------|--------|-------------------|
| FOMC rate decision | Meetings in March, May | If unexpected cut/hike, consider rapid-response LinkedIn post connecting to that week's theme |
| Major PE deal in orthopedics | Ongoing | If announced during May, integrate into Week 10–12 content angles |
| Congressional tax action | Ongoing | If OBBBA modification proposed, assess impact on all Roth/estate content |
| CMS reimbursement updates | Ongoing | If additional cuts announced, accelerate Week 3 content or add signal-driven piece |
| IRS guidance on OBBBA implementation | Ongoing | Review all April tax content for accuracy; revise if needed |
