...
**Check the literal text, not your memory.** Before finalizing each file, actually search its body for dash characters `—` (em dash, U+2014), `–` (en dash, U+2013), and `―` (horizontal bar), and for `not … but` / `not … it's` pivots. Any hit is a defect to fix, not to wave through. The Quality Checklist you write must reflect this real re-derivation: never mark "No em dashes: PASS" on a body that still contains one.
...
0. **Final em-dash sweep (BLOCKING, do this before writing anything).** For every file about to be written, literally scan the full text for `—` (U+2014), `–` (U+2013), and `―`. If any file contains one, it is not done: fix it, then re-run `voice-check` on that file. **Never write, export, or report a batch that still contains an em dash.** Also confirm `voice-check` actually ran on every piece in Step 5; if it did not run on even one, run it now before proceeding. This sweep is the last line of defense and is not optional.
