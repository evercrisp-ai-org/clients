...
- **Mitigation:** Two layers. (1) `research-scan` now actively **researches the live web** before generation, verifying each figure against authoritative sources (IRS, SSA, Treasury), updating stale values, and citing the source and date. (2) Guardrail G12 (validate Relevance check) re-checks: any rate / limit / legal fact still flagged "confirm before publish" stays **Yellow**, never Green. So figures are both refreshed from primary sources and held for a human confirm.
- **New sub-risk it introduces:** a web source could itself be stale, wrong, or secondary. Mitigated by `research-scan` preferring primary/authoritative sources, citing every figure, and **still flagging rate-sensitive facts for human confirmation** rather than trusting the fetch blindly. Never treat a researched figure as publish-ready without the human confirm.
- **Residual risk:** Low-Medium. Figures are now actively verified against primary sources instead of merely flagged, which lowers the chance of a stale number reaching a draft. The human confirm of flagged rate-sensitive facts before publish remains the backstop. **This is still the most important manual step in the pipeline.**
